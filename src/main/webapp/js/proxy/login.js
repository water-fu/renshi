$(document).ready(function(){
	...
	// 判断是否有cookie
	var accountCookie = $.cookie('/proxy');
	if(accountCookie) {
	...
	$('#login').click(function() {
		...
		if(errorInput.length > 0) {
			...
			// 提交表单
			$('#loginForm').attr('action', path + '/proxy/login');
			...
			$.ajaxFormSubmit({
				...
				success: function(data) {
					...
					if(checkUser) {
						// 有效期7天的cookie
						$.cookie('/proxy', data.obj.loginAccount, { expires: 7 });
					}
					// 不记住
					else {
						// 删除 cookie
						$.cookie('/proxy', '', { expires: -1 });
					}
					
					var index = location.href.indexOf(path);
					var realPath = location.href.substring(index);
					if(realPath == path || realPath == path + '/proxy/' || realPath == path + '/proxy') {
	...
	$('#toRegister').click(function() {
		location.href = path + '/proxy/registerPage';
	});
